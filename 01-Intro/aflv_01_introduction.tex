\ifx \handoutstyle \undefined
  \def\handoutstyle{} %alternativ: mitloesung
\fi
\documentclass[12pt,t,\handoutstyle]{beamer}
\subtitle{01 -- Introduction}
\newcommand{\shorttitle}{Introduction}
...
\begin{frame}{}
	\vfill
	\begin{center}
		{\color{hilight}There's no such thing as a stupid question.}\\
    	If you have questions: Interrupt and \underline{ask}!
	\end{center}
	\vfill
\end{frame}
...
\begin{frame}{What is Competitive Programming?}
    \vspace{40pt}
    \bi
		\item ... the art of solving complex, well-defined algorithmical or mathematical problems within a given time bound (both for coding and program runtime) 
...
\item Multi-level competition
		\bi
			...
			\item World Finals - between March and June
		\ei
\ei
\end{frame}
...
\begin{frame}{What is the ACM ICPC?}
		\begin{center}
			...
			\only<2>{\includegraphics[width = \textwidth]{images/DSC02167.JPG}}
			\only<3>{\includegraphics[width = \textwidth]{images/team.jpg}}
			\only<4>{\includegraphics[width = \textwidth]{images/wf.jpg}}
			\only<5>{\includegraphics[width = 0.95\textwidth]{images/wc.jpg}}
			\only<6>{\includegraphics[width = \textwidth]{images/bjarne.jpg}}
			\only<7>{\includegraphics[width = \textwidth]{images/DSC01457.JPG}}
			\only<8>{\includegraphics[width = \textwidth]{images/DSC01573.JPG}}
			\only<9>{\includegraphics[width = \textwidth]{images/DSC02081.JPG}}
			\only<10>{\includegraphics[width = \textwidth]{images/DSC02078.JPG}}
			\only<11>{\includegraphics[width = \textwidth]{images/DSC02949.JPG}}
			\only<12>{\includegraphics[width = \textwidth]{images/IMG_4275.JPG}}
			\only<13>{\includegraphics[width = \textwidth]{images/IMG_6791.JPG}}
		\end{center}
\end{frame}
...
\begin{frame}{Why train Competitive Programming?}
    \bi
		...
		\item Learn to tackle complex problems systematically.
		\item Learn to see patterns in problems.
		\item Learn perseverance when facing (seemingly) impossible problems.
		\item Learn to design algorithms that always work.
		\item Learn to de-bug code.
		\item Learn to develop code in teams and under pressure.
    ...
    \bi
        \item Given a problem, we want to
            \bi
                ...
                \item by using appropriate algorithms and data structures,
...
\begin{frame}{How?}
    ...
    \bi
        ...
        \item Introduce new algorithms, data structures, and common theory
        \bi
		  \item Have you heard about \texttt{map}, \texttt{set}, \texttt{priority\_queue} or \texttt{dequeue}?
		  \item What about Union Finds, Fenwick-Trees, Suffix Arrays?
		  \item What about Dijkstra, Push-Relable, Gift-Wrapping, 2-SAT?
		  \item What about Pick's, Burnside?
		  \item There is \underline{a lot} more
        \ei
        \item Practice problem solving
        \item Practice programming
        \item<2-> More practice\pause\pause, more practice\pause, more practice \dots
    \ei
\end{frame}
...
\begin{frame}{Technical Matters}
		\begin{tabular}{l l}
			Lecture & Thursdays 16:00 - 17:30 (O28/121)\\
			Tutorial & Thursdays 17:30 - 19:00 (Süd-Pool)\\
			Changes  & no lecture on 4.7. and 11.7.\\
					& additional lecture: 29.4.\\
					& 30.5. moved to 27.5.\\
					& 20.6. moved to 17.6.\\
					& (all Monday, O27/121)\\
...
\begin{frame}{Technical Matters -- Tutorial}
		\bi
			...
			\item Solving a problem is worth
			\bi
				\item very easy - 2 points
				\item easy - 3 points ($\approx 2-3$ per week)
				\item medium - 4 points ($\approx 3-2$ per week)
				\item hard - 5 points ($\approx 1$ per week)
				\item very hard - 6 points
				\item last semester: 267 points in total
			\ei
			\item Marks are based on total points as follows
			\bi
				\item 4.0 -- $33,\overline{3}\%$ of the points (last semester 178)
				\item 1.0 -- $66,\overline{6}\%$ of the points (last semester 89)
...
\begin{frame}{Technical Matters -- Problems}
	\vspace*{-0.4cm}
	\bi
		\item All problems are ``competition-style'' problems, which ask to compute an output given a specified input.
		\item You hand in your solutions to a judging system.
		\bi
			\item Submissions are judged immediately and automatically	
			\item The judge has a set of (secret) testcases
			\item You only get points for a problem if the judge accepts your solution, i.e., if it is correct on all testcases, does not crash, and finishes in time.
			...
			\item The judge accepts C(++), Java, Python3, Haskell, Lua, Go, Prolog, D, and x86 assembler
		\ei
		\item Judge URL \url{https://mandos.informatik.uni-ulm.de}
		\item Login on the judge using your SIG-Pool account
		\item All solutions will be checked for plagiarism using an automated checker after the hand-in deadline
	\ei
\end{frame}


\begin{frame}{Technical Matters -- GCPC}
	\bi
		\item The GCPC will take place on \\
				Saturday, July 6\textsuperscript{th}, probably 11:00-16:00.
...
\begin{frame}{Course book}
    ...
    \bi
		...
		\item Third edition is available in the library \\
				\url{https://ulm.ibs-bw.de/aDISWeb/app?service=direct/0/Home/$DirectLink&sp=S127.0.0.1:23002&sp=SAKSWB-IdNr398882932}
    \ei
\end{frame}


\begin{frame}{Semester schedule}
	\begin{tabular}{l l}
			25.4. & Introduction I\\
			29.4. & Introduction II\\
			02.5. & Simple Algorithms \\
			09.5. & Datastructures \\
			16.5. & Problem Solving Paradigms \\
			23.5. & Greedy \\
			27.5. & Dynamic Programming I \\
			06.6. & Dynamic Programming II \\
			13.6. & Unweighted Graphs \\
			17.6. & Weighted Graphs \\
			27.6. & Geometry \\
			06.7. & GCPC \\
			18.7. & Mathematics\\
			25.6. & Strings
    ...
    \bi
        ...
        \item Usually consists of
            \bi
				...
				\item (sometimes) A memory limit in (mega/mebi) bytes
            \ei
...
\begin{frame}{Judge verdicts}
    \bi
		...
		\vspace{2cm}
	\item<2-> It is important to learn to debug your own code without knowing the error!
    \ei
\end{frame}
...
\begin{frame}{Tip 2: Do Algorithm Analysis}
    \bi
        \item When solving a problem, our solution has to be fast enough and cannot use too much memory
...
\begin{frame}{Tip 2: Do Algorithm Analysis}
    \bi
		...
		\item<2-> Try to prive it is not!
        \item<3-> Even if you don't manage to prove or disprove it, you will probably get a better understanding of the problem
        \vspace{20pt}
    \ei
\end{frame}
...
\begin{frame}{Tip 5: Practice, practice, and more practice}
    \vspace{30pt}
    \bi
...
\begin{frame}{Programming Language}
	\bi
		\item So far, programming was easy and we could have used any language!
		\item Not all languages are suited for all problems
		\bi
			\item Time-critial operations in Java?
			\item Operating System in Haskell?
			\item GUI in Brainfuck?
			\item Exact numerics in C++?
			\item Banking software in Cobol?
		\ei
		\item A good programmer knows his tools!
	\ei
\end{frame}
...
\begin{frame}{Programming Language}
    ...
    \bi
        \item I'll use C++ 
		\bi
			\item Java is useful for its BigInteger/BigDecimal classes
			\item Python is useful for String manipulation
		\ei
	  ...
	  \item nor correct (you'll understand if you dive deeper into the language)
        \ei
		\item in the ICPC only C, C++, Java, Python, and (possibly) Kotlin are allowed
		\item Our judge Accepts
			\bi
	\item C, C++, Java, Python 3
	\item Haskell, Prolog, Lua
	\item x86-Assembler
			\ei
    \ei
\end{frame}
...
\begin{frame}[fragile]{C++ for Java Programmers}
    ...
    \bi
      ...
      \item Programs start at \texttt{main}
	  \begin{minted}[fontsize=\scriptsize]{cpp}
...
#include <map>
        \end{minted}
        \item Everything is in \texttt{bits/stdc++.h}
        \begin{minted}[fontsize=\scriptsize]{cpp}
#include <bits/stdc++.h>
        \end{minted}
...
\begin{frame}[fragile]{C++ for Java Programmers}
    \bi
        ...
        \bi
			...
			\item \texttt{\_\_int128\_t}: a 128-bit signed integer
			\vspace{5pt}
...
if (i = j) do_something();
        \end{minted}
        \ei
	\item There is also the keyword \texttt{auto}
    \ei
\end{frame}
...
\begin{frame}[fragile]{C++ for Java Programmers}
    ...
    \bi
	  ...
	  \item You can declare functions \emph{outside} of classes\\
			  (e.g. \texttt{std::sort})
...
\begin{frame}[fragile]{Templates}
  \bi
  	\item Do you know that templates are?
	\item<2-> Templates look like Generics in Java, but are completely different
	\item<2-> If you have lists of \texttt{int}, of \texttt{long}, of \texttt{char}, and of lists of \texttt{int}, you may have to write the code four times ...
	\item<2-> Templates are a way to abstract from parameter types
	\item<2-> You write your list code once for an unknown type \texttt{T}
	\item<2-> If someone uses your lists, he has to tell the compiler the actual type \texttt{T}
	\item<2-> You already know one very similar case: Arrays!
	\item<2-> \texttt{int[]} $\rightarrow$ \texttt{vector<int>}
	\item<2-> Array could be read as \texttt{[]<int>}
  \ei
\end{frame}

\begin{frame}[fragile]{Templates and the STL}
  \bi
   \item The standard library is called\\
		   Standard \textbf{Template} Library
   \item It provides a lot of useful functions and container types, most of them similar to Java's containers
   \item Function names \emph{can} be a bit unintuitive
...
\begin{frame}{What I have not talked about}
		Pointers!
		\bi
\item<2-> Pointers are explicit representations of memory adresses
\item<2-> Notation \texttt{int*} is an adress at which an \texttt{int} is stored.
\item<2-> \texttt{int**} is an adress at which an adress of an \texttt{int} is stored.
\item<2-> Pointers are the way of C(++) to handle dynamic memory and dynamic data structures
		\vspace{30pt}
\item<3-> Pointers can be \textbf{dangerous}
\item<3-> We will use them rarely
		\ei
\end{frame}
...
struct tree {
    tree *left, *right; int value; tree(){left = right = 0;}
};
tree t;
...

{
    \small
    \vspace{20pt}
Company XYZ has been badly hit by recession and is taking a lot of cost cutting measures. Some of these measures include giving up office space, going open source, reducing incentives, cutting on luxuries and issuing pink slips.

\vspace{10pt}

They have got three employees working in the accounting department and are going to lay-off two of them.
...
You will be given the salaries of these 3 employees working in the accounting department. You have to find out the salary of the person who survives.
}
...
\begin{frame}[fragile]{Solution: Cost Cutting}
    \begin{minted}[fontsize=\scriptsize]{cpp}
#include <cstdio>
#include <algorithm>
using namespace std;
...
have limited number of keys. To accommodate all alphabets, multiple letters are
compressed into single keys. Therefore, to type certain characters, a key
...
\begin{frame}[fragile]{Solution: SMS Typing}
    \begin{minted}[fontsize=\scriptsize]{cpp}
#include <iostream>
...
\begin{frame}[fragile]{Solution: Hardwood Species}
    \begin{minted}[fontsize=\scriptsize]{cpp}
int main(){
    int tc; cin >> tc;
...
\begin{frame}[fragile]{Solution: February 29}
    \begin{minted}[fontsize=\scriptsize]{cpp}
