...
\begin{frame}{What is Competitive Programming?}
    \vspace{40pt}
    \bi
		\item .. the art of solving complex, well-defined problems quickly within a given time-bound (both for coding and program runtime) 
		\pause
...
\begin{frame}{Why learn about Competitive Programming?}
    \vspace{40pt}
    \bi
...
\begin{frame}{Technical Matters -- Problems}
	\bi
		...
		\bi
			...
			\item You only get points for a problem if the judge accepts your solution, i.e., if it is corrent on all inputs, does not crash and finished in time.
			\item There are no partial points
			\item You can submit your solution as often as you like
			\item Malicious attacks will be penalised
...
\begin{frame}{Programming Language}
    ...
    \bi
	  ...
	  \item nor correct (you'll see if you dive deeper into the language)
        \ei
      \item only C, C++, Java, and Python are allowed
    \ei
\end{frame}
...

{
    ...
    \begin{tabular}{c|c|c}
        ...
        $\leq 10^7$ & $O(n), O(\log_2{n}), O(1)$ & everything else (greedy, ad hoc) \\
    \end{tabular}
    \end{center}
}
