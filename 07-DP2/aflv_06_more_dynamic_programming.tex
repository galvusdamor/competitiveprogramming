...
\begin{frame}{DP with probabilities}
	\vspace{40pt}
    \bi
        \item Many stochastic problems are in their core DP problems
...
\begin{frame}{DP with probabilities}
    ...
    \bi
        ...
        \item You are playing alone
        \item<2-> If you play perfectly, how many truns do you need on average to uncover the hole memory
        \vspace{10pt}
        \item<3-> What are the DP dimensions?
        \item<4-> $h = $ How may cards are left
        \item<5-> $k = $ How may cards we know
        \vspace{10pt}
        \item<6-> $\mathrm{dp}(0,0) = 0$
        \item<6-> $\mathrm{dp}(h,k) = 1 + \mathrm{dp}(h-2,k-2)$ \quad iff $k > \frac{h}{2}$
        \item<6-> $\mathrm{dp}(h,k) = ...$ \quad iff $k > \frac{h}{2}$
    \ei
\end{frame}
